(function () {
  ...
  function removeClass (element, uselessClass) {
    element.className.replace(uselessClass, '')
  }

  function loadMySchedules (mySchedules) {
    for (var day in mySchedules) {
      for (var id in mySchedules[day]) {
        var meeting = '<li id="' + id + '">' +
        '<p class="meeting-name">' + mySchedules[day][id].name + ' </p>' +
        '<p class="meeting-location"> Room <br>' + mySchedules[day][id].room + '</p>' +
        '<p class="meeting-info">' + ' Observers allowed ' + '</p>' +
        '<button class="rsvp-button" data-scheduleid="' + id + '"><img src="../assets/img/add-button.svg" alt=""></button>' +
        '</li>'
        document.getElementById('my-' + day + '-schedule').innerHTML += meeting
      }
    }
  }
  ...
  function loadSchedules (schedules) {
    for (var day in schedules) {
      for (var id in schedules) {
        var meeting = '<li id="' + id + '">' +
        '<p class="meeting-name">' + schedules[day][id].name + ' </p>' +
        '<p class="meeting-location"> Room <br>' + schedules[day][id].room + '</p>' +
        '<p class="meeting-info">' + ' Observers allowed ' + '</p>' +
        '<button class="rsvp-button" data-scheduleid="' + id + '"><img src="../assets/img/add-button.svg" alt=""></button>' +
        '</li>'
        document.getElementById(day + '-schedule').innerHTML = meeting
      }
    }
  }

  function displaySchedule (currentSchedule, newSchedule) {
    document.getElementById(currentSchedule + '-schedule').style.display = 'none'
    document.getElementById(newSchedule + '-schedule').style.display = ' '
  }

  function init () {
    var schedulesNavBtns = document.getElementsByClassName('schedule-navigation-btn')
    var currentTab = schedulesNavBtns[0]
    for (var index in schedulesNavBtns) {
      var btn = schedulesNavBtns[index]
      btn.addEventListener('click', function () {
        displaySchedule(currentTab.innerHTML.toLowerCase(), btn.innerHTML.toLowerCase())
        btn.addClass('active')
  ...
  if (!window.localStorage.mySchedules) {
    window.localStorage.setItem('mySchedules', '{}')
  } else {
    init()
    loadSchedules(window.localStorage.schedules)
    loadMySchedules(JSON.parse(window.localStorage.mySchedules))
  }
